...
resource "aws_instance" "segment_7" {
  ami                  = "${data.aws_ami.segment_7.id}"
  instance_type        = "i3.xlarge"
  key_name             = "gp_secondary"
  placement_group      = "${aws_placement_group.greenplum.id}"
  iam_instance_profile = "${element(aws_iam_instance_profile.instance_profile.*.id, 8)}"
  monitoring           = true
  ebs_optimized        = true
...
export S3_bucket=`aws --region eu-west-2 ssm get-parameter --name GP_SG7_S3_BACKUP --query 'Parameter.Value' --output text --with-decryption`
...
EOF
  ...
  tags {
    Name = "segment-7-${local.naming_suffix}"
  }

  network_interface {
    device_index         = 0
    network_interface_id = "${aws_network_interface.segment_7_0.id}"
  }

  network_interface {
    device_index         = 1
    network_interface_id = "${aws_network_interface.segment_7_1.id}"
  }

  network_interface {
    device_index         = 2
    network_interface_id = "${aws_network_interface.segment_7_2.id}"
  }

  network_interface {
    device_index         = 3
    network_interface_id = "${aws_network_interface.segment_7_3.id}"
  }
...
resource "aws_network_interface" "segment_7_0" {
...
resource "aws_network_interface" "segment_7_1" {
...
resource "aws_network_interface" "segment_7_2" {
...
resource "aws_network_interface" "segment_7_3" {
